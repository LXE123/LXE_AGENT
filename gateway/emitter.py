...
class GatewayEmitter:
    ...
    async def emit(self, emit: EmitRequest) -> None:
        ...
        response_route_id = str(emit.response_route_id or "").strip()
        route_ctx = await load_response_route_context(response_route_id) if response_route_id else None
        ...
        if emit_kind == "tool":
            await self._send_files(adapter, emit=emit, platform=platform, response_route_id=response_route_id)
            if emit.content:
                await self._send_message(adapter, emit=emit, platform=platform, response_route_id=response_route_id, content=emit.content)
            return
