...
test("Yacang IPC accepts only public account setup and bounds passwords", () => {
  const input = { workspace_root: "/workspace", yacang: { action: "save", mobile: "phone", password: "secret" } };
  expect(validateSetupInput(input).yacang).toEqual(input.yacang);
