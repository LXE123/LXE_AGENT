...
const setup = () => {
  ...
  writeFileSync(join(root, "runtime", "python", "python.exe"), "python-binary", "utf8");
  ...
  writeFileSync(manifestPath, `${JSON.stringify({
    ...
    files: [
      ...
      { path: "runtime/python/python.exe", size: 13, sha256: sha256("python-binary"), owner: "managed-python-runtime", policy: "immutable", integrity: "sha256" },
    ],
  })}\n`, "utf8");
...
describe("desktop resource integrity", () => {
  ...
  test("reports the exact managed Python path before runtime startup", async () => {
    const { root, manifestPath } = setup();
    rmSync(join(root, "runtime", "python", "python.exe"));
    await expect(verifyDesktopResourceManifest(root, manifestPath, "win32-x64"))
      .rejects.toThrow("Desktop resource file is missing: runtime/python/python.exe");
  });
});
