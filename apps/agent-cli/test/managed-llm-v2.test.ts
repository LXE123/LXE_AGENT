...
test("published alternatives enforce model membership and freeze each acquired turn's credentials", async () => {
  ...
  const environment = { AGENT_LLM_PROVIDER: "deepseek", AGENT_LLM_MODEL: "deepseek-v4-flash", AGENT_LLM_CREDENTIAL_SOURCE: "cloud" };
  let state: ManagedLlmState = { revision: 1, default_target: { provider: "deepseek", model: "deepseek-v4-flash" },
    models: ["deepseek-v4-flash", "deepseek-v4-pro"].map((model, i) => ({ provider: "deepseek", model, available: true, credential_revision: (i ? "b" : "a").repeat(64) })),
    credentials: ["deepseek-v4-flash", "deepseek-v4-pro"].map((model, i) => ({ provider: "deepseek", model, api_key: "key-" + model, credential_revision: (i ? "b" : "a").repeat(64), fetched_at: 1, invalid_revision: "" })) };
  const used: string[] = [];
  ...
  try {
    ...
    expect(list.items.filter((m) => m.credential_source === "cloud").map((m) => m.model)).toEqual(["deepseek-v4-flash", "deepseek-v4-pro"]);
    expect(JSON.stringify(list)).not.toContain("key-deepseek");
    const first = manager.acquire();
    await service.call({ operation: "models.update", input: { provider: "deepseek", model: "deepseek-v4-pro", credential_source: "cloud" } });
    expect(environment.AGENT_LLM_MODEL).toBe("deepseek-v4-pro");
    ...
    expect(used).toEqual(["key-deepseek-v4-flash", "key-deepseek-v4-pro"]);
    await expect(blocked.provider.turn(request)).rejects.toThrow("unavailable");
    Object.assign(environment, { LXE_MANAGED_LLM_PROVIDER: "future_vendor", LXE_MANAGED_LLM_MODEL: "future-model" });
    environment.AGENT_LLM_MODEL = "deepseek-v4-flash";
    const unsupportedDefault = await manager.reconfigure({});
    await expect(unsupportedDefault.provider.turn(request)).rejects.toThrow("unavailable");
    await service.call({ operation: "models.update", input: { provider: "deepseek", model: "deepseek-v4-flash", credential_source: "cloud" } });
    await manager.acquire().provider.turn(request);
    expect(used.at(-1)).toBe("key-deepseek-v4-flash");
