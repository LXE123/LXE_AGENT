...
await runtime.start();
for (const id of ["a", "b"]) await runtime.ensureSession({ session_id: id, source: { platform: "desktop" }, workspace });
let failAnswer = false, delayAnswer = false;
const vite = await createServer({ root: resolve("apps/dashboard"), server: { port: 5201, strictPort: true, host: "127.0.0.1" }, plugins: [{
      ...
      try {
        ...
        if (raw.operation === "fixture.start") {
          ...
          await scheduler.enqueue({ ...jobFixture, job_id: crypto.randomUUID(), session_id: id, message_id: crypto.randomUUID(),
            user_input: "请帮我处理店铺", source: { platform: "desktop" }, response_route_id: "", workspace });
          result = true;
          ...
          else if (call.operation === "sessions.answer" || call.operation === "sessions.questions" || call.operation === "sessions.detail") {
...
console.log("Question acceptance: http://127.0.0.1:5201/test/features/sessions/user-questions-fixture.html");
