...
description: 需要理解紫鸟浏览器 WebDriver 自动化接口的定位、能力、架构或对接流程时使用。
type: ziniao_browser
references:
  - path: references/api-core.md
    description: 核心工作流接口（applyAuth/getBrowserList/startBrowser）
  - path: references/api-auxiliary.md
    description: 辅助管理接口（stopBrowser/logout/exit/缓存/插件/updateCore）
  - path: references/startup-params.md
    description: 启动客户端的命令行参数详细说明
  - path: references/prerequisites.md
    description: 权限开通、账号配置、常见问题排查
---
