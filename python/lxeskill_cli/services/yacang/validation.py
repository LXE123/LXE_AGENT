...
def validate_inventory_sales_workbook(path: Path, *, warehouse_code: str) -> int:
    try:
        workbook = load_workbook(path, read_only=True, data_only=True)
    except Exception as exc:  # noqa: BLE001 - expose the real parser failure
        raise YacangError("校验 XLSX", f"{type(exc).__name__}: {safe_remote_detail(exc)}") from exc
    try:
...
def validate_inventory_list_workbook(path: Path, *, warehouse_code: str) -> int:
    try:
        workbook = load_workbook(path, read_only=True, data_only=True)
    except Exception as exc:  # noqa: BLE001 - expose the real parser failure
        raise YacangError("校验库存列表 XLSX", f"{type(exc).__name__}: {safe_remote_detail(exc)}") from exc
    try:
        if len(workbook.sheetnames) != 1:
            raise YacangError(
                "校验库存列表 XLSX",
        ...
        if headers != INVENTORY_LIST_HEADERS:
            raise YacangError("校验库存列表 XLSX", f"表头不匹配: {safe_remote_detail(headers)}")
        row_count = 0
        wrong_warehouses: set[str] = set()
        for row in rows:
            if not any(value is not None for value in row):
...
def validate_inbound_listing_time_workbook(path: Path) -> int:
    try:
        workbook = load_workbook(path, read_only=True, data_only=True)
    except Exception as exc:  # noqa: BLE001 - expose the real parser failure
        raise YacangError("校验仓库产品 XLSX", f"{type(exc).__name__}: {safe_remote_detail(exc)}") from exc
    try:
        if len(workbook.sheetnames) != 1:
            raise YacangError(
                "校验仓库产品 XLSX",
        ...
        rows = sheet.iter_rows(values_only=True)
        headers = tuple(str(value or "").strip() for value in next(rows, ()))
        if headers != INBOUND_LISTING_TIME_HEADERS:
            raise YacangError("校验仓库产品 XLSX", f"表头不匹配: {safe_remote_detail(headers)}")
        row_count = 0
        invalid_times = 0
        for row in rows:
            if not any(value is not None for value in row):
