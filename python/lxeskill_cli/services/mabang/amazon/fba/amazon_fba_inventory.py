...
DEFAULT_SNAPSHOT_DIR = dataset_dir("replenish_fba_inventory_snapshots")
SOURCE = "amazon_fba_inventory_snapshot"
AMAZON_FBA_INVENTORY_SNAPSHOT_FILE_SUFFIX = "亚马逊物流库存快照"
SUMMARY_SHEET = "亚马逊物流库存汇总"
DETAIL_SHEET = "亚马逊物流库存明细"
VALIDATION_SHEET = "校验摘要"
AMAZON_FBA_TOTAL_COLUMN = "FBA 总库存（亚马逊物流库存）"
...
class AmazonFbaInventoryValidationSummary:
    marketplace: str
    mabang_site: str
...
@dataclass(frozen=True)
class AmazonFbaInventorySnapshotResult:
    store_name: str
    snapshot_time: str
    snapshot_date: str
    ...
    validation: AmazonFbaInventoryValidationSummary
    source: str = SOURCE
    ...
    def to_payload(self) -> dict[str, Any]:
        return {
            ...
            "amazon_fba_inventory_validation": self.validation.to_payload(),
            "source": self.source,
        }


@dataclass(frozen=True)
class AmazonFbaInventorySnapshotData:
    store_name: str
    snapshot_date: str
...
def _csv_records(source_path: Path) -> list[dict[str, Any]]:
    if not source_path.is_file():
        raise FileNotFoundError(f"亚马逊物流库存 CSV 不存在: {source_path}")
    ...
    if not records:
        raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存 CSV 没有数据: {source_path}")
    headers = set(records[0].keys())
    missing = [column for column in REQUIRED_CSV_COLUMNS if column not in headers]
    if missing:
        raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存 CSV 缺少列: {', '.join(missing)}")
    return records
...
def _snapshot_date_text(value: Any) -> str:
    ...
    for fmt in ("%Y-%m-%d", "%Y/%m/%d", "%Y%m%d"):
        try:
            ...
            continue
    raise AmazonFbaInventorySnapshotError(f"无法解析亚马逊物流库存 snapshot-date: {text}")
...
def _snapshot_time_text(snapshot_date: str, value: str | None = None) -> str:
    ...
    if text:
        if not re.fullmatch(r"\d{12}", text):
            raise AmazonFbaInventorySnapshotError(f"snapshot_time 必须是 12 位时间: {text}")
        return text
...
def _mabang_msku_snapshot(
    ...
    *,
    ...
    try:
        ...
        if missing:
            raise AmazonFbaInventorySnapshotError(f"店铺MSKU数据缺少列: {', '.join(missing)}")
    ...
    except AmazonFbaInventorySnapshotError:
        raise
    except Exception as exc:
    ...
    if not msku_values:
        raise AmazonFbaInventorySnapshotError(f"店铺MSKU数据没有可用于校验的 MSKU: {source_path}")
...
def _aggregate_amazon_rows(records: list[dict[str, Any]], *, store_name: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]], str, str]:
    ...
    if len(snapshot_dates) != 1:
        raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存 snapshot-date 不唯一: {', '.join(sorted(snapshot_dates)) or '空'}")
    marketplaces = {_marketplace_code(record.get("marketplace")) for record in records if _clean_text(record.get("marketplace"))}
    if len(marketplaces) != 1:
        raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存 marketplace 不唯一: {', '.join(sorted(marketplaces)) or '空'}")
    ...
    if not summary_by_sku:
        raise AmazonFbaInventorySnapshotError("亚马逊物流库存 CSV 未解析到有效 sku")
...
def _validate_snapshot(
    *,
...
) -> AmazonFbaInventoryValidationSummary:
    ...
    if match_ratio < SKU_MATCH_RATIO_THRESHOLD:
        raise AmazonFbaInventorySnapshotError(
            "亚马逊物流库存文件疑似不是当前店铺/站点数据: "
    ...
    return AmazonFbaInventoryValidationSummary(
...
def write_amazon_fba_inventory_snapshot(
    summary_rows: list[dict[str, Any]],
    detail_rows: list[dict[str, Any]],
    validation: AmazonFbaInventoryValidationSummary,
    ...
    try:
        ...
        raise RuntimeError("缺少 openpyxl 依赖，无法写入亚马逊物流库存快照") from exc
...
def build_amazon_fba_inventory_snapshot(
    csv_path: str | Path,
    *,
    store_name: str,
    ...
    target_path = _snapshot_dir(output_dir) / f"{timestamp}-{_safe_path_part(clean_store_name)}_{AMAZON_FBA_INVENTORY_SNAPSHOT_FILE_SUFFIX}.xlsx"
    write_amazon_fba_inventory_snapshot(summary_rows, detail_rows, validation, target_path)
    return AmazonFbaInventorySnapshotResult(
        store_name=clean_store_name,
...
def _records_from_xlsx(path: str | Path, sheet_name: str) -> list[dict[str, Any]]:
    ...
    if not source_path.is_file():
        raise FileNotFoundError(f"亚马逊物流库存快照不存在: {source_path}")
    try:
        from openpyxl import load_workbook
    except Exception as exc:
        raise RuntimeError("缺少 openpyxl 依赖，无法读取亚马逊物流库存快照") from exc

    workbook = None
    try:
        workbook = load_workbook(source_path, read_only=True, data_only=True)
        if sheet_name not in workbook.sheetnames:
            raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存快照缺少 sheet: {sheet_name}")
        ...
        raise RuntimeError(f"读取亚马逊物流库存快照失败: {source_path}, error={exc}") from exc
    finally:
        try:
            if workbook is not None:
                workbook.close()
...
def _validation_from_records(records: list[dict[str, Any]]) -> AmazonFbaInventoryValidationSummary | None:
    if not records:
        return None
...
def load_amazon_fba_inventory_snapshot(path: str | Path, *, store_name: str | None = None) -> AmazonFbaInventorySnapshotData:
    ...
    if missing:
        raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存快照缺少列: {', '.join(missing)}, path={path}")
    ...
    if clean_store_name and stores and clean_store_name not in stores:
        raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存快照中未找到店铺: {clean_store_name}")
    dates = {_clean_text(row.get("快照日期")) for row in records if _clean_text(row.get("快照日期"))}
    if len(dates) != 1:
        raise AmazonFbaInventorySnapshotError(f"亚马逊物流库存快照日期不唯一: {', '.join(sorted(dates)) or '空'}")
    ...
    return AmazonFbaInventorySnapshotData(
...
__all__ = [
    "AMAZON_FBA_TOTAL_COLUMN",
    "AMAZON_FBA_INVENTORY_SNAPSHOT_FILE_SUFFIX",
    "AmazonFbaInventorySnapshotData",
    "AmazonFbaInventorySnapshotError",
    "AmazonFbaInventorySnapshotResult",
    "AmazonFbaInventoryValidationSummary",
    "DEFAULT_SNAPSHOT_DIR",
    "SOURCE",
    ...
    "write_amazon_fba_inventory_snapshot",
]
