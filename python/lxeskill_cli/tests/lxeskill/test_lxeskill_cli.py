...
def test_catalog_defines_every_cli_command_and_hidden_alias() -> None:
    ...
    assert sum(bool(entry.get("module")) for entry in catalog.values()) == 39
    ...
    assert all(
        ...
        if not name.startswith(("browser_auth_", "shangman_", "yacang_", "mabang_tms_", "mabang_brazil_"))
    )
...
def test_list_and_help_write_one_terminal_jsonl_record(capsys) -> None:
    ...
    assert len(records[0]["data"]["commands"]) == 43
