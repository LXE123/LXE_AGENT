---
name: feishu-im-read
description: 飞书 IM bot 会话读取指南，覆盖当前或指定 bot 已加入群聊的历史消息、话题回复以及图片/文件资源下载。优先用于 LXE 当前飞书群上下文里的只读消息查看；不要用于发送消息、跨群搜索、群管理、用户/机器人身份切换或其他应由 lark-im CLI skill 处理的飞书操作。
type: default
---
...
- 用户提到当前或指定 bot 可见群里的“聊天记录”、“消息”、“群里说了什么”、“话题回复”、“图片”或“文件下载”。
- 这是只读、bot 可见会话工具；发送消息、跨群/全局搜索、群管理、Feed/标记、反应管理、用户/机器人身份切换等操作交给 `lark-im`。
