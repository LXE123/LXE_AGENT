...
- 说明厂家名包含 `正飞` 的明细行会填写同厂家跨明细加权 `均价 = sum(原价 * 数量) / sum(数量)`，并四舍五入保留两位小数，再填写 `售价(均价) = 均价 / 含税倍率 / (1 - 毛利率)`；非正飞行这两个字段留空。
- 说明 `备货单` 最后一行是带填充色的 `合计` 行，会汇总 `数量`、`总价` 和 `总价（售价）`。
- 说明输出表格所有列宽和行高已统一为 15。
- 说明 `备货单` 字段为 `库存sku`、`产品名称`、`库存sku（第一行）`、`产品名称（第一行）`、`型号`、`原价`、`均价`、`售价`、`售价(均价)`、`毛利率`、`厂家`、`单位`、`合同产品名称`、`数量`、`总价`、`总价（售价）`。
