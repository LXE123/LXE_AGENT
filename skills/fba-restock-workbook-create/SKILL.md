...
- 说明 `备货单` 已按 `型号` 合并，但不同厂家相同型号会保留为不同行；同型号多个库存 SKU 会在 `库存sku` 单元格中按 `SKU × 数量` 分行显示，`产品名称` 按相同顺序分行显示，并额外提供纯 SKU 的 `库存sku（第一行）` 和 `产品名称（第一行）` 用于筛选或复制代表值。
...
- 说明 `备货单` 字段为 `日期`、`库存sku（第一行）`、`产品名称（第一行）`、`型号`、`数量`、`原价`、`厂家`、`采购订单号`、`总价（原价）`、`合同产品名称`、`均价`、`售价`、`总价（均价）`、`总价（售价）`、`单位`、`毛利率`、`库存sku`、`产品名称`、`售价(均价)`、`总价（售价(均价)）`。
