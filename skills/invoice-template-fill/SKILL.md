...
- 发票明细按 `箱号 + 财务合并后的库存SKU` 写入；财务合并规则来自备货单第 2 个 sheet 的 `规则型号 + 单价`，保留 SKU 以第 3 个 sheet `汇总表` 为准。
- 备货单第 2 个 sheet 中相同 `SKU + 规则型号 + 单价` 的重复行会自动累加；同一 SKU 出现不同 `规则型号` 或 `单价` 时视为异常。
