...
1. 先运行 `uv run --frozen python "$SKILL_DIR/scripts/template_tool.py" search --query "<主题>" --limit 3`，根据用户描述的 **场景、风格、色调** 做初筛
...
# 先找候选模板
uv run --frozen python "$SKILL_DIR/scripts/template_tool.py" search --query "工作汇报" --tone light --limit 3

# 看指定页型的紧凑摘要
uv run --frozen python "$SKILL_DIR/scripts/template_tool.py" summarize --template office--work_report --label 内容

# 只裁切目标页型，避免把整份 XML 拉进上下文
mkdir -p artifacts/lark_work/<unique-task-name>
uv run --frozen python "$SKILL_DIR/scripts/template_tool.py" extract --template office--work_report --label 封面 --out artifacts/lark_work/<unique-task-name>/work-report-cover.xml
```
