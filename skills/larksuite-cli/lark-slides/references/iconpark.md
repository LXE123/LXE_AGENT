...
uv run --frozen python "$SKILL_DIR/scripts/iconpark_tool.py" search --query "增长趋势" --limit 8
uv run --frozen python "$SKILL_DIR/scripts/iconpark_tool.py" resolve --name chart-line
uv run --frozen python "$SKILL_DIR/scripts/iconpark_tool.py" list-categories
```
