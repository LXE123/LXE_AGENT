...
- 可简要转述 `sku_count`、`sku_source_count`、`matched_sku_count`、`unmatched_sku_count`、`manufacturer_count`。
- 如果 `warnings` 非空，必须转述给用户；例如出口退税总表存在完全相同的重复库存 SKU 且已自动去重，或存在 `库存sku` 为空的行且已忽略。
...
- 说明 `采购汇总` 和厂家 sheet 字段为 `库存sku`、`产品名称`、`来源SP单号`、`型号`、`原价`、`厂家`、`数量`、`总价`。
