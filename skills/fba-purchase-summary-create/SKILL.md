...
- 说明厂家名包含 `正飞` 的明细行会填写同厂家跨明细加权 `均价 = sum(原价 * 数量) / sum(数量)`，并四舍五入保留两位小数；非正飞行 `均价` 留空。
- 说明 `采购汇总` 和厂家 sheet 最后一行是带填充色的 `合计` 行，会汇总 `数量` 和 `总价`。
- 说明输出表格所有列宽和行高已统一为 15。
- 说明 `采购汇总` 和厂家 sheet 字段为 `库存sku`、`产品名称`、`来源SP单号`、`库存sku（第一行）`、`产品名称（第一行）`、`型号`、`原价`、`均价`、`厂家`、`单位`、`合同产品名称`、`合同编号前缀`、`税率`、`数量`、`总价`。
