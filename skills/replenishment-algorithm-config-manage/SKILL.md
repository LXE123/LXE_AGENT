...
- 海运最低重量已取消，缺少重量也不阻断海运；旧参数中的 `sea.min_weight_kg` 不再生效，新配置表和可编辑参数列表不再展示此项。仅多出旧重量字段的上一版配置表仍可导入，无需修改历史文件。
...
- 更早版本中 sheet 结构或必需字段不兼容的配置表仍需重新 `export`；不要仅因含旧重量字段就判断配置表不兼容。
