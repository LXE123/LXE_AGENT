...
- 仅读取源表中确认 Active 的行，保留其销量和本地 SKU 绑定。复用下载时保存的 Listing 绑定和 SKU 类型（1 库存、2 组合），不重复读取 Listing；只实时查询参与计算的去重组合明细和深圳库存。
- 需要桌面端配置 `LXE_DATA_SERVER_URL` 和 `LXE_DATA_SERVER_API_KEY`；深圳仓库查询仍需要马帮登录态。
- 大店铺可能需要较长时间，命令总超时为 30 分钟；进度写入 stderr，以最终 terminal 为准。
- 源表缺少核验信息、被改动、核验绑定冲突或组合明细缺失时停止生成报告，按实际错误处理；不会回退到网页组合导出。未确认 Active 的行留在源表，不进入库存分类；没有 Active 行时明确返回无符合条件的记录。
...
- 结果文件包含 4 个业务 Sheet，以及隐藏的 `Active核验信息` Sheet。业务 Sheet 为：`真实库存（深圳仓库）-组合sku`、`真实库存（深圳仓库）-库存sku`、`无本地SKU`、`无库存数据`。
...
- `unique_local_sku_count` = Active 范围内有本地 SKU 的去重本地 SKU 数。
...
- `original_row_count`、`active_row_count`、`excluded_row_count` 记录源表范围；其余库存数量统计只覆盖 Active 行。
