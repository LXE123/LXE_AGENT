...
def test_fba_purchase_summary_skill_documents_contract() -> None:
    ...
    assert "`备货单` 字段为 `日期`、`库存sku（第一行）`、`产品名称（第一行）`、`型号`、`数量`、`原价`、`厂家`、`采购订单号`、`总价（原价）`、`合同产品名称`、`均价`、`售价`、`总价（均价）`、`总价（售价）`、`单位`、`毛利率`、`库存sku`、`产品名称`、`售价(均价)`、`总价（售价(均价)）`" in text
...
def test_fba_restock_workbook_skill_documents_contract() -> None:
    ...
    assert "`总价（售价(均价)） = 售价(均价) * 数量`" in text
    assert "`13%` 按含税倍率 `1.13`" in text
    assert "售价四舍五入保留两位小数" in text
    assert "总价（售价）" in text
    assert "最后一行是带填充色的 `合计` 行，会汇总 `数量`、`总价（原价）`、`总价（均价）`、`总价（售价）` 和 `总价（售价(均价)）`" in text
    assert "`库存sku`、`产品名称`、`售价(均价)` 和 `总价（售价(均价)）` 位于最后四列，并使用灰色填充" in text
    ...
    assert "`日期`、`库存sku（第一行）`、`产品名称（第一行）`、`型号`、`数量`、`原价`、`厂家`、`采购订单号`、`总价（原价）`、`合同产品名称`、`均价`、`售价`、`总价（均价）`、`总价（售价）`、`单位`、`毛利率`、`库存sku`、`产品名称`、`售价(均价)`、`总价（售价(均价)）`" in text
    assert "所有列宽和行高已统一为 15" in text
