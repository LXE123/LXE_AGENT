...
def _result(snapshot_path: str) -> AmazonFbaInventorySnapshotResult:
...
def test_success_returns_snapshot_payload(monkeypatch, capsys, tmp_path) -> None:
    ...
    def fake_build_amazon_fba_inventory_snapshot(
        csv_path_arg,
        *,
        store_name,
        output_dir=None,
    ...
    monkeypatch.setattr(cli, "build_amazon_fba_inventory_snapshot", fake_build_amazon_fba_inventory_snapshot)
    ...
    assert payload["amazon_fba_inventory_validation"]["top_inventory_matched_count"] == 8
    assert payload["source"] == "amazon_fba_inventory_snapshot"


def test_build_error_returns_failure_json(monkeypatch, capsys) -> None:
    def fake_build_amazon_fba_inventory_snapshot(*args, **kwargs):
