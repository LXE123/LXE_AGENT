...
def _event(*, user_id: str, app_id: str) -> InboundEvent:
    return InboundEvent(
        platform="feishu",
        connector_key="agent",
...
def test_router_denies_unknown_bot_without_enqueue(monkeypatch) -> None:
    ...
    decision = asyncio.run(router.route_message(_event(user_id=USER_LYX, app_id="cli_unknown")))
...
def test_router_denies_user_without_agent_access(monkeypatch) -> None:
    ...
    decision = asyncio.run(router.route_message(_event(user_id=USER_ZGL, app_id=BOT_ID_LXE_CLAW)))
...
def test_router_allows_authorized_user_and_bot(monkeypatch) -> None:
    ...
    decision = asyncio.run(router.route_message(_event(user_id=USER_ZGL, app_id=BOT_ID_LXE_FBA_AGENT)))
