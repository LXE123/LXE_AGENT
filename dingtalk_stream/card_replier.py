...
from shared.infra.net import dingtalk_http_session, dingtalk_requests_session
...
class CardReplier(object):
    ...
    @staticmethod
    def _sync_request_text(method: str, url: str, **kwargs) -> str:
        response = dingtalk_requests_session.request(method, url, **kwargs)
        response_text = response.text
        response.raise_for_status()
        return response_text

    @staticmethod
    async def _async_request_text(method: str, url: str, **kwargs) -> str:
        async with dingtalk_http_session.request(method, url, **kwargs) as response:
    ...
    def create_and_send_card(
        ...
        response_text = ""
        try:
            response_text = self._sync_request_text(
                "POST",
                url,
    ...
    async def async_create_and_send_card(
        ...
        response_text = ""
        try:
            response_text = await self._async_request_text(
                "POST",
                url,
                headers=self.get_request_header(access_token),
                json=body,
